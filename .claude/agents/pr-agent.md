...
4. Fill in `.github/PULL_REQUEST_TEMPLATE.md`'s sections (배경 및 개요 / 작업내용 / 리뷰노트 / 체크리스트 / 기타), then print the draft

## Rules

- Do not include anything not in the diff
- Only check checklist items you have actually verified (실제로 `npm test`/`npm run test:e2e` 실행 결과를 확인한 경우에만 체크)
